...
> **Amendment (2026-07-24, execution start — interface drift, verified by probing
> the installed library at Plans 10–19 state).** The plan below was written
> against the *intended* post-Plan-08 surface; the REAL surface differs. Every
> task executor MUST apply these substitutions wherever the original task text
> conflicts; they are the Task-1 freshness-gate findings, resolved:
>
> 1. **Version floor:** `MIN_QUIVERLAB_VERSION = "0.1.0"` (installed:
>    `0.1.0.dev0`; the 0.9.0 floor was aspirational — Plan 08 did not bump).
> 2. **`families()`** returns a `FamilyListing` of `FamilyInfo` records with
>    fields `name`, `signature` (display string), `route`, `citations`,
>    `summary` — NOT `(name, builder)` pairs. The builder is
>    `getattr(quiverlab, info.name)` (all listed names are top-level exports);
>    parameters come from `inspect.signature(builder)`, skipping `field`.
>    **Exclude the `zoo` entry from the buildable catalog** (it yields a batch
>    of algebras, not one; `zoo(dim_max=12, field=None)` is not a v1 family).
> 3. **Reference family everywhere** (`truncated_polynomial(n=2, field=GF(2))`
>    does not exist in `families()`): use **`QuantumCI(q=1, field=GF(2))`**
>    (dim 4; q=1 stays nondegenerate in char 2 — never use q=2 over GF(2),
>    where 2 ≡ 0 degenerates the relation). In schema-only tests any family
>    string is fine but use `"QuantumCI"` for consistency.
> 4. **`A.dimension()` does not exist** — it is the attribute **`A.dim`**.
> 5. **HH tables have NO `.latex()`.** The real contract (and the worker's
>    result-block shape, per the Plan-10 amendment = `docs/gui/runner.py`
>    `compute_one`): call `A.hochschild_cohomology(top, verbose=False,
>    trace=events)` / `hochschild_homology(...)`; read `table.kind`,
>    `table.dims`, `table.engine`, `table.references`; citation pairs via
>    `quiverlab.trace.provenance.resolve_references(tuple(keys))`; per-algebra
>    keys via `A.citations()`; `global_dimension()` returns an object with
>    `.exact`/`.value` and a rich `str()`; `center()` returns `(dim, basis)`
>    with basis entries JSON-shipped via `str()`; matrices LaTeX'd by the
>    runner's `_latex_matrix`; trace HTML via `quiverlab.trace.render_html`.
> 6. **`quiverlab.verbose` defaults to `True`** and HH calls emit worked-steps
>    PDFs into `quiverlab_traces/` — the server/worker MUST pass
>    `verbose=False` on every library call that accepts it (as the GUI runner
>    does) and never rely on the global flag.
> 7. **Schema v1 accepts BOTH `kind: "family"` and `kind: "quiver"`** (Plan-10
>    amendment (b) is now in force): `Literal["family", "quiver"]`, with the
>    quiver payload exactly as `docs/gui/runner.py::run_build` consumes it
>    (`vertices: list`, `arrows: {name: [s, t]}`, `relations: [str]`); family
>    fields are required only when `kind == "family"`, quiver fields only when
>    `kind == "quiver"`. Relation strings are parsed by the library's exact
>    relation grammar (loud `RelationError` — no code execution).
> 8. **`bibliography()`** returns a `Bibliography` with `.groups`
>    (`{topic: [Entry]}` — `Entry` has `key`, `bibtex_key`, `formatted`,
>    `annotation`, `doi`, `arxiv`, `topic`), `.keys`, `.bibtex() -> str`,
>    `.to_dict()`. The `/literature` page renders `.groups`; the downloadable
>    BibTeX is `.bibtex()`.
>
> Where a task's inline code contradicts the above, the amendment wins; keep
> the task's structure, tests-first flow, and commit boundaries.
