...
The suite is **2846 tests** (collected with the `[dev,fast,docs,web,qpa,hpc]` extras,
2026-08-05, after Plans 21–33, the Plan-38 C2 forms/type/positive-roots/recognizer
batteries + Koszulity (`ext_algebra`) exposure, the Plan-32 oracle-class markers + audit gate,
...
The live QPA suite is `-m qpa` (126 tests). GAP is heavy to install, so it runs in a
...
| `invariants/` (Cartan, Coxeter, spectral, Betti, cyclic, Frobenius incl. the Plan-29 trace-form symmetry certifier, scalar, sweep; Plan-29 Coxeter/identity literature batteries; **the Plan-38 C2 layer** — `forms.py` Euler/Tits forms + finite/tame/wild definiteness, `dynkin_type.py` orientation-blind ADE/affine detection, `roots.py` positive-root enumeration, `recognizers.py` the semisimple…gentle batch + the shared no-code block builders) | 173 | fast | second models (λ-complex, relative-Tor Betti); self-certifying `λ`/`ν`; GF(p) engine parity; **Plan-38 literature pins** (arXiv:2606.15595 Coxeter examples, Gabriel forms, Bourbaki root counts, textbook gentle/string) + **cross-engine** (type detection round-tripped against the `families/dynkin` generator and cross-checked against the Tits-form signature) |
...
| `qpa/` (GAP/QPA crosscheck) | 147 | 126 qpa + 21 fast | **live GAP/QPA** (HH dims, self-Ext, τ/τ⁻, proj/inj resolutions, inj dim, Plan-31 native trivial-extension construction — left side via `A^op`, Plan-38 `IsSpecialBiserialAlgebra`/`IsGentleAlgebra`); script builders + guards run without GAP |
...
|---|---|---:|---|
| Literature / theory pins | `-m oracle_literature` | 773 | the engine reproduces a value/identity that exists outside the library |
| Cross-engine agreement | `-m oracle_crossengine` | 463 | two independent implementations compute the same thing and match live |
...
| Live QPA / GAP | `-m qpa` | 126 | an independent external system (QPA) recomputes and agrees |
| Any oracle class (union) | `-m "oracle_literature or oracle_crossengine or oracle_selfcert or qpa"` | 1642 | the test is pinned by at least one oracle (the remaining tests are contract/infrastructure) |
