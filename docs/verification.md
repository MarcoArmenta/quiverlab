...
The live QPA suite is `-m qpa` (127 tests). GAP is heavy to install, so it runs in a
...
| Cross-engine agreement | `-m oracle_crossengine` | 437 | two independent implementations compute the same thing and match live |
| Self-certifying certificates | `-m oracle_selfcert` | 886 | an internal axiom (d∘d=0, canonicality, an arbitration identity) holds by construction |
| Live QPA / GAP | `-m qpa` | 137 | an independent external system (QPA) recomputes and agrees |
| Any oracle class (union) | `-m "oracle_literature or oracle_crossengine or oracle_selfcert or qpa"` | 1641 | the test is pinned by at least one oracle (the remaining tests are contract/infrastructure) |

Recounted 2026-08-05 from live collection for **Plan 39** (complexes, chain maps,
cones, hyper-Ext) — the counts above are this branch's live `pytest --collect-only`
numbers (the mid-merge-train dev page was stale; the orchestrator does a final
recount after all Wave merges). Plan 39 adds 13 `oracle_selfcert`
(`tests/modules/test_complexes_core.py` + `test_complexes_cone.py`), 7
`oracle_crossengine` (`test_complexes_hyperhom.py` + `test_complexes_model.py`),
and 10 `qpa` (`tests/qpa/test_complexes_qpa.py`) tests. The oracle markers live only
on the pure-library `engine` / `resolutions_cs` / `hochschild` / `modules` /
`invariants` / `families` / `batch` / `trace` suites (the `trace` renderer tests
import the pure-library serializers only, and their `hpc.spec` uses are
function-local), so these counts do **not** depend on the `[web]`/`[hpc]` extras.

**Plan 39 — GUI story deferred to P42/P43.** The complex / chain-map / cone /
hyper-Ext surface is the SUBSTRATE for spectral sequences (P42) and the derived
category (P43); the metaplan assigns the user-facing GUI/report surfaces of those
tools to their own plans. So Plan 39 ships the public library API
(`from quiverlab import ChainComplex, ChainMap`; `Algebra.chain_complex`) and its
oracles, but NO GUI/webapp panel — that arrives with P42/P43, which consume this
API as specified. This deferral is recorded here (the standing rule is honored by
the named successor, not silently skipped) and in the plan's final commit message.
