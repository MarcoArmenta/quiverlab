...
The suite is **2906 tests** (collected with the `[dev,fast,docs,web,qpa,hpc]` extras,
...
The live QPA suite is `-m qpa` (130 tests). GAP is heavy to install, so it runs in a
...
| `deep` | 1328 | one Linux · py3.12 cell, **twice**: numba and pure (`QUIVERLAB_NO_NUMBA=1`) |
| `qpa` | 130 | weekly Linux · py3.12 job with GAP + QPA (`QUIVERLAB_REQUIRE_QPA=1`) |
...
| Self-certifying certificates | `-m oracle_selfcert` | 873 | an internal axiom (d∘d=0, canonicality, an arbitration identity) holds by construction |
| Live QPA / GAP | `-m qpa` | 130 | an independent external system (QPA) recomputes and agrees |
| Live Macaulay2 | `-m m2` | 10 | an independent external system (Macaulay2) recomputes and agrees |
| Any oracle class (union) | `-m "oracle_literature or oracle_crossengine or oracle_selfcert or qpa or m2"` | 1683 | the test is pinned by at least one oracle (the remaining tests are contract/infrastructure) |
