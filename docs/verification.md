...
The suite is **3028 tests** (collected with the `[dev,fast,docs,web,qpa,hpc]` extras,
...
The live QPA suite is `-m qpa` (160 tests). GAP is heavy to install, so it runs in a
...
| `webapp/` (server tier + result cache + offline GUI — non-algebraic glue) | 429 | fast | API / schema / cache canonicalizer (replay-safety rests on exactness) / isolation / artifacts; all math delegated to the library; Plan-28 runner delegation pinned **byte-identical** (frozen goldens + unchanged `canonical_key`) |
...
| `deep` | 1408 | one Linux · py3.12 cell, **twice**: numba and pure (`QUIVERLAB_NO_NUMBA=1`) |
| `qpa` | 160 | weekly Linux · py3.12 job with GAP + QPA (`QUIVERLAB_REQUIRE_QPA=1`) |
...
|---|---|---:|---|
| Literature / theory pins | `-m oracle_literature` | 806 | the engine reproduces a value/identity that exists outside the library |
| Cross-engine agreement | `-m oracle_crossengine` | 472 | two independent implementations compute the same thing and match live |
...
| Live QPA / GAP | `-m qpa` | 160 | an independent external system (QPA) recomputes and agrees |
| Live Macaulay2 | `-m m2` | 10 | an independent external system (Macaulay2) recomputes and agrees |
| Any oracle class (union) | `-m "oracle_literature or oracle_crossengine or oracle_selfcert or qpa or m2"` | 1793 | the test is pinned by at least one oracle (the remaining tests are contract/infrastructure) |
...
- **Basic-ization / Gabriel recovery is char-scoped and split-only** (Plan 44 C7).
  ...
  trace-form radical, which is rigorous ONLY over characteristic 0 or characteristic
  > dim A (the same Dickson/Cohen–Ivanyos–Wales bound `decompose` uses) — off scope they
  refuse with a loud `QuiverlabError` (`M₂(k)` over GF(2) is a tested refusal), never a
  silent wrong idempotent set. And the semisimple quotient must be SPLIT: a genuine
  non-split division-algebra Wedderburn block (e.g. a quaternion corner over QQ) is out
  of scope and refuses loudly. The recovery is certified per instance (dimension identity
  + a multiplicativity check that the reconstructed `kQ/I → B` is an algebra iso).
- **`repetitive_slice` ships certified FINITE slices only** (Plan 44 C7). The full
  ...
  built (certified by `dim == (2·copies−1)·dim A`). `copies < 1` refuses loudly. The
  Hughes–Waschbüsch orbit identity `T(A) ≅ hat(A)/(ν)` (a slice with periodic
  identification) is a NAMED SUCCESSOR, not shipped — no unverified orbit number.
- **`JacobianAlgebra` refuses Jacobian-infinite inputs** (Plan 44 C7) with
  ...
  `W = 0` — is a tested refusal), never a fabricated finiteness claim. The surface QPs
  of Labardini-Fragoso (whose Jacobian-finiteness is a per-surface theorem) are the
  named successor (P48); this plan pins only the hand-derived triangle (`dim 6`) and the
  `Jac(3-cycle, abc) ≅ kZ₃/J²` cross-engine identity.
- **The preprojective algebra of a Dynkin quiver is NOT a Jacobian algebra of its
  double** (Plan 44 C7, corrected from the plan draft). The cyclic derivative of the
  commutator potential `Σ_a (a a* − a* a)` is identically 0 (`∂_a(a a* − a* a) = a* − a*
  = 0`), and the double of a tree is bipartite, hence has no oriented 3-cycles, so no
  cubic potential exists to produce the quadratic mesh relations. The Jacobian
  cross-engine oracle is therefore `Jac(3-cycle, abc) ≅ cyclic Nakayama kZ₃/J²`, not the
  (false) preprojective identity the plan proposed.
- **Tilting complement MUTATION is deferred** (Plan 44 C7): `bongartz_completion` ships
  ...
  the plan's anticipated honest skip, Plan-44 CROSSCHECKS against both (`tests/qpa/
  test_tilting_qpa.py`).
