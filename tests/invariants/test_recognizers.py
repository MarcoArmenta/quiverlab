...
def test_gentle_is_ideal_invariant_on_non_minimal_presentation():
    # Devil's-advocate regression (2026-08-05): a*b*c lies in (a*b), so the
    # ideal IS generated by the single length-2 path a*b and kA4/(ab) is
    # gentle -- the recognizer must read the ideal (reduction system), never
    # the raw generator list a user happened to type.
    A = Quiver([1, 2, 3, 4], {"a": (1, 2), "b": (2, 3), "c": (3, 4)}).algebra(
        relations=["a*b", "a*b*c"], field=GF(5))
    assert rec.is_string(A) is True
    assert rec.is_gentle(A) is True
