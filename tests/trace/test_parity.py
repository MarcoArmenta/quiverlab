"""Renderer parity (spec §8): the SAME events produce CONSISTENT claims in both
renderers -- identical resulting dims and identical per-degree ranks. The binding
discipline holds regardless of output format (text console / HTML report)."""
import re
...
def test_both_renderers_agree_on_dims():
    ev, table = _events()
...
def test_both_renderers_agree_on_homology_dims():
    """Homology mirror of the cohomology parity test (review C1): both renderers
    must derive the engine's HOMOLOGY dims. For k[x]/(x^2) the correct dims are
    [2, 1, 1]; the pre-fix cohomology formula (rk[n-1]) yielded [2, 2, 1] on these
    homology events, so this test fails under the old derive_dims."""
...
def test_both_render_the_same_ranks():
    ev, table = _events()
...
def test_both_carry_the_reference():
    ev, table = _events()
    for render in (render_text, render_html):
